...
/*--------- Messages ---------*/
 ...
 if (message.message.text) {
     ...
     botly.sendText({
       id: senderId,
       text: 'كود الدعوة غير صالح 😑\n تأكد ان صديقك ارسل لك الكود بشكل صحيحلكود بشكل صحيح',
...
function initiateMultiplayerGame(senderId, totalRounds) {
  ...
  setTimeout(() => {
    botly.sendText({
        id: senderId,
        text: `ارسل هذا الكود الى صديقك وقل له ان يدخل لصفحتي ويضغط على زر 'ادخال كود الدعوة' ويرسله وسيبدأ اللعب بينكما \n\nتنتهي مدة صلاحية الدعوة بعد 5 دقائق او يمكنك انهاؤها عبر الزر`
    });
}, 1000);
  setTimeout(() => {
    botly.sendText({
